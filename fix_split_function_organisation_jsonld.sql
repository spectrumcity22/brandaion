-- Fix the split_questions_into_review function to use the correct organisation_jsonld_object field

-- First, let's check what the correct field name is in construct_faq_pairs
SELECT 
    column_name, 
    data_type
...
-- Now update the function to use the correct field
            ...
            IF length(clean_question) > 10 AND clean_question ~ '^\[.+?\]\[.+?\]$' THEN
                ...
                IF topic_name IS NOT NULL AND question_text IS NOT NULL AND length(topic_name) > 0 AND length(question_text) > 0 THEN
                    INSERT INTO review_questions (
                        ...
                        NEW.organisation_jsonld_object
                    );
                END IF;
            END IF;
        END LOOP;
