...
export default function FAQPerformancePage() {
  ...
  const testFAQPerformance = async () => {
    ...
    // Calculate actual remaining questions (excluding already selected ones)
    const actualRemaining = userStats.questionsRemaining - selectedPairs.length;
    ...
    // Check if selected questions exceed remaining quota
    if (selectedPairs.length > userStats.questionsRemaining) {
      setError(`You can only select up to ${userStats.questionsRemaining} more questions for this month (you have ${userStats.questionsAsked} already asked, ${userStats.questionsLimit} total limit)`);
      return;
    }
